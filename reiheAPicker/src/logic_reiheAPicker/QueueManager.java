...
public class QueueManager {
	...
	private void removeShortDDCs()
	{
		...
		{		
				if (!this.queueTitles.get(counter).get(this.ddcIndex).equals("-") && !this.queueTitles.get(counter).get(this.ddcIndex).isEmpty())
				{
						...
						if ((this.queueTitles.get(counter).get(this.ddcIndex).contains("B") && this.propertyFileHandler.propertyFileModel.get_settings_BlockBelletristik().equals("true"))
								||(this.queueTitles.get(counter).get(this.ddcIndex).contains("K") && this.propertyFileHandler.propertyFileModel.get_settings_RemoveJuvenileLiterature().equals("true"))
								||(this.queueTitles.get(counter).get(this.ddcIndex).contains("S") && this.propertyFileHandler.propertyFileModel.get_settings_RemoveSchoolBooks().equals("true")))
						{
							System.out.println("removed belletristic book: " + this.queueTitles.get(counter).get(0));
							this.queueTitles.get(counter).set(1, "Aufgrund DDC aussortiert! (K/S/B)");
	...
	private void removeDuplicatesInQueue()
	{
		...
		if (this.propertyFileHandler.propertyFileModel.get_settings_RemoveDuplicatesByTitle().equals("true"))
		{
			for (int outerCounter = 0; outerCounter <= this.queueTitles.size()-1; outerCounter ++)
			{
				for (int innerCounter = 0; innerCounter <= this.queueTitles.size()-1; innerCounter ++)
				{
						if (innerCounter != outerCounter && this.queueTitles.get(outerCounter).get(0).equals(this.queueTitles.get(innerCounter).get(0)))
						{
							System.out.println("removed duplicate (byTitle): " + this.queueTitles.get(innerCounter).get(0) + " <---|---> " + this.queueTitles.get(outerCounter).get(0));
							this.queueTitles.get(innerCounter).set(1, "Als Duplikat anhand des Titels aussortiert!");
							this.nopeTitles.add(this.queueTitles.get(innerCounter));
							this.queueTitles.remove(this.queueTitles.get(innerCounter));
							if (innerCounter > 0)
							innerCounter --;
	...
	{		
		...
		for (int counter = 0; counter < currentTitle.size(); counter++)
		{
			if (currentTitle.get(counter).equals("-") || currentTitle.get(counter).isBlank() || currentTitle.get(counter).isEmpty())
			{
				//dont show empty columns?
			}
			else 
